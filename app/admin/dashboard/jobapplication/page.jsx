...
import ApplicationMoadal from './viewModal';
...
function JobApplication() {
  ...
  const GetBlog = async (token) => {
    setLoader(true)
    try {
      const res = await readData(`application/getAllapplication`, {
        headers: {
  ...
  const columns = [
    ...
    { field: "fullName", headerName: "Candidate Name", width: 180 },
...

    {
      ...
      width: 200,
      renderCell: (params) =>
        params.row?.jobDetails?.jobTitle || "-",
    },

    {
      ...
      width: 150,
      renderCell: (params) =>
        params.row?.jobDetails?.department || "-",
    },
  ...
  return (
    ...
    <div>
      ...
      <ApplicationMoadal handleCloseView={handleCloseView} openView={openView} setOpenView={setOpenView} selectedApplication={selectedApplication}/>
    </div>
  )
}
