...
import JobModal from './jobeditmodal';
...
function JobList() {
  ...
  const GetAllJobsopening = async (token) => {
    setLoader(true)
    try {
...
console.log("jobId",jobId)
  ...
  useEffect(() => {
    const fetchData = async () => {
      const token = localStorage.getItem("token");
      await GetAllJobsopening(token);  // ✔️ now state update happens asynchronously
    };
  ...
  return (
    ...
    <div>
      ...
      <JobModal openView={open} setOpenView={setOpen} jobId={jobId} handleCloseView={handleCloseView} GetAllJobsopening={GetAllJobsopening}/>
    </div>
  )
}
